...
export const TenantsScreen: React.FC<TenantsScreenProps> = ({ navigation }) => {
  ...
  const clearFilters = () => {
    // Reset all filter states but don't reload data yet
    // User will need to click Apply to reload with cleared filters
    setStatusFilter("ALL");
    setSelectedRoomId(null);
    setPendingRentFilter(false);
    setPendingAdvanceFilter(false);
        ...
        <View style={{ flexDirection: "row", gap: 8 }}>
          ...
          >
            <Ionicons
              name="search"
              size={18}
              color={isTyping ? "#fff" : Theme.colors.text.tertiary}
            />
          </TouchableOpacity>
          {!!appliedSearch && (
            <TouchableOpacity
