...
export const SlideBottomModal: React.FC<SlideBottomModalProps> = ({
  ...
  const clampedMaxPercent = Math.min(
    1,
    Math.max(clampedMinPercent, maxHeightPercent)
  );
  const minH = Math.min(
    maxSheetH,
    Math.max(200, Math.round(screenH * clampedMinPercent))
  );
  const maxH = Math.min(
    maxSheetH,
    Math.max(minH, Math.round(screenH * clampedMaxPercent))
  );
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        if (enableFlexibleHeightDrag) {
          ...
          const next = Math.max(
            minH,
            Math.min(maxH, startHeight - gestureState.dy)
          );
