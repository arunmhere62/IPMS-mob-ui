    ...
    <>
      ...
      <View style={[styles.heroCard, isPaid ? styles.heroPaid : isPending ? styles.heroPending : styles.heroOverdue]}>
        ...
        <View style={styles.heroBottom}>
          ...
          <View style={[styles.heroBadge, isPaid ? styles.badgePaid : isPending ? styles.badgePending : styles.badgeOverdue]}>
            ...
            <Text style={[styles.heroBadgeText, isPaid ? styles.textPaid : isPending ? styles.textPending : styles.textOverdue]}>{raw?.payment_status ?? 'N/A'}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.statsRow}>
        <View style={styles.statCard}>
          ...
          <View style={styles.statContent}>
            ...
            <Text style={styles.statValue} numberOfLines={1} ellipsizeMode="tail">{raw?.rooms?.room_no ?? 'N/A'}</Text>
          </View>
        </View>
        <View style={styles.statCard}>
          ...
          <View style={styles.statContent}>
            ...
            <Text style={styles.statValue} numberOfLines={1} ellipsizeMode="tail">{raw?.beds?.bed_no ?? 'N/A'}</Text>
          </View>
        </View>
        <View style={styles.statCard}>
      ...
      {raw?.unpaid_months && raw.unpaid_months.length > 0 ? (
        <View style={styles.alertCard}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.alertTitle}>Unpaid Month{raw.unpaid_months.length > 1 ? 's' : ''}</Text>
          ...
          <View style={styles.alertBadge}>
            <Text style={styles.alertBadgeText}>{raw.unpaid_months.length}</Text>
          </View>
        </View>
      ) : null}
      ...
      <View style={styles.sectionCard}>
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionTitle}>My PG</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons name="location-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel}>Address</Text>
          <Text style={styles.infoValue}>{raw?.pg_locations?.address ?? 'N/A'}</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons name="map-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel}>City</Text>
          <Text style={styles.infoValue}>{raw?.pg_locations?.city?.name ?? 'N/A'}</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons name="refresh-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel}>Cycle Type</Text>
          <Text style={styles.infoValue}>{raw?.pg_locations?.rent_cycle_type ?? 'N/A'}</Text>
        </View>
        <View style={[styles.infoRow, { borderBottomWidth: 0 }]}>
          <Ionicons name="log-in-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel}>Check-in</Text>
          <Text style={styles.infoValue}>{formatDate(raw?.check_in_date)}</Text>
        </View>
      </View>
            ...
            <View key={p.s_no} style={[styles.payRow, index === raw.rent_payments.slice(0, 3).length - 1 ? { borderBottomWidth: 0 } : null]}>
              ...
              <View style={{ alignItems: 'flex-end', gap: 3 }}>
                ...
                <View style={[styles.statusBadge, p.status === 'PAID' ? styles.statusPaid : p.status === 'PARTIAL' ? styles.statusPartial : styles.statusPending]}>
                  <Text style={[styles.statusText, p.status === 'PAID' ? styles.textStatusPaid : p.status === 'PARTIAL' ? styles.textStatusPartial : styles.textStatusPending]}>{p.status}</Text>
                </View>
              </View>
            </View>
          ))
