    ...
    <>
      ...
      <View style={[styles.heroCard, isPaid ? styles.heroPaid : isPending ? styles.heroPending : styles.heroOverdue]}>
        <View style={styles.heroTop}>
          <View>
            <Text style={styles.heroGreeting} numberOfLines={1} adjustsFontSizeToFit>Hello, {raw?.name?.split(' ')[0] ?? 'Tenant'} 👋</Text>
            <Text style={styles.heroSub} numberOfLines={1} adjustsFontSizeToFit>{raw?.pg_locations?.location_name ?? 'My PG'}</Text>
          </View>
        ...
        <View style={styles.heroBottom}>
          <View>
            <Text style={styles.heroAmountLabel} numberOfLines={1} adjustsFontSizeToFit>Due Amount</Text>
          ...
          <View style={[styles.heroBadge, isPaid ? styles.badgePaid : isPending ? styles.badgePending : styles.badgeOverdue]}>
            ...
            <Text style={[styles.heroBadgeText, isPaid ? styles.textPaid : isPending ? styles.textPending : styles.textOverdue]} numberOfLines={1} adjustsFontSizeToFit>{raw?.payment_status ?? 'N/A'}</Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.statsRow}>
        <View style={styles.statCard}>
          ...
          <View style={styles.statContent}>
            <Text style={styles.statLabel} numberOfLines={1} adjustsFontSizeToFit>Room</Text>
            <Text style={styles.statValue} numberOfLines={1} ellipsizeMode="tail" adjustsFontSizeToFit>{raw?.rooms?.room_no ?? 'N/A'}</Text>
          </View>
        </View>
        <View style={styles.statCard}>
          ...
          <View style={styles.statContent}>
            <Text style={styles.statLabel} numberOfLines={1} adjustsFontSizeToFit>Bed</Text>
            <Text style={styles.statValue} numberOfLines={1} ellipsizeMode="tail" adjustsFontSizeToFit>{raw?.beds?.bed_no ?? 'N/A'}</Text>
          </View>
        </View>
        <View style={styles.statCard}>
          ...
          <View style={styles.statContent}>
            <Text style={styles.statLabel} numberOfLines={1} adjustsFontSizeToFit>Rent</Text>
            <Text style={styles.statValue} numberOfLines={1} ellipsizeMode="tail" adjustsFontSizeToFit>{formatAmount(raw?.beds?.bed_price)}/mo</Text>
          </View>
        </View>
      </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 12, color: raw?.expected_vacate_date ? '#8B5CF6' : Theme.colors.text.secondary, marginTop: 2, fontWeight: raw?.expected_vacate_date ? '700' : '400' }} numberOfLines={1} adjustsFontSizeToFit>
          ...
          >
            <Text style={{ fontSize: 12, fontWeight: '700', color: '#8B5CF6' }} numberOfLines={1} adjustsFontSizeToFit>
      ...
      {ticketStats?.overview && (
        <View style={styles.sectionCard}>
          <View style={styles.sectionHeader}>
            ...
            <Text style={styles.sectionTitle} numberOfLines={1} adjustsFontSizeToFit>My Tickets</Text>
          </View>
          <View style={styles.ticketStatsRow}>
            <View style={styles.ticketStatItem}>
              ...
              <Text style={styles.ticketStatLabel} numberOfLines={1} adjustsFontSizeToFit>Total</Text>
            </View>
            <View style={styles.ticketStatItem}>
              <Text style={[styles.ticketStatValue, { color: '#f59e0b' }]}>{ticketStats.overview.open}</Text>
              <Text style={styles.ticketStatLabel} numberOfLines={1} adjustsFontSizeToFit>Open</Text>
            </View>
            <View style={styles.ticketStatItem}>
              <Text style={[styles.ticketStatValue, { color: '#3b82f6' }]}>{ticketStats.overview.inProgress}</Text>
              <Text style={styles.ticketStatLabel} numberOfLines={1} adjustsFontSizeToFit>In Progress</Text>
            </View>
            <View style={styles.ticketStatItem}>
              <Text style={[styles.ticketStatValue, { color: '#10b981' }]}>{ticketStats.overview.resolved}</Text>
              <Text style={styles.ticketStatLabel} numberOfLines={1} adjustsFontSizeToFit>Resolved</Text>
            </View>
          </View>
        </View>
      )}
      ...
      {raw?.unpaid_months && raw.unpaid_months.length > 0 ? (
        <View style={styles.alertCard}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.alertTitle} numberOfLines={1} adjustsFontSizeToFit>Unpaid Month{raw.unpaid_months.length > 1 ? 's' : ''}</Text>
            {raw.unpaid_months.map((m: { cycle_start: string; cycle_end: string }, i: number) => (
              <Text key={i} style={styles.alertSub} numberOfLines={1} adjustsFontSizeToFit>
                {formatDate(m.cycle_start)} – {formatDate(m.cycle_end)}
              </Text>
            ))}
          </View>
          <View style={styles.alertBadge}>
            <Text style={styles.alertBadgeText} numberOfLines={1} adjustsFontSizeToFit>{raw.unpaid_months.length}</Text>
          </View>
        </View>
      ) : null}
      ...
      <View style={styles.sectionCard}>
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionTitle} numberOfLines={1} adjustsFontSizeToFit>My PG</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons name="location-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel} numberOfLines={1} adjustsFontSizeToFit>Address</Text>
          <Text style={styles.infoValue} numberOfLines={1} adjustsFontSizeToFit>{raw?.pg_locations?.address ?? 'N/A'}</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons name="map-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel} numberOfLines={1} adjustsFontSizeToFit>City</Text>
          <Text style={styles.infoValue} numberOfLines={1} adjustsFontSizeToFit>{raw?.pg_locations?.city?.name ?? 'N/A'}</Text>
        </View>
        <View style={styles.infoRow}>
          <Ionicons name="refresh-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel} numberOfLines={1} adjustsFontSizeToFit>Cycle Type</Text>
          <Text style={styles.infoValue} numberOfLines={1} adjustsFontSizeToFit>{raw?.pg_locations?.rent_cycle_type ?? 'N/A'}</Text>
        </View>
        <View style={[styles.infoRow, { borderBottomWidth: 0 }]}>
          <Ionicons name="log-in-outline" size={16} color="#94a3b8" />
          <Text style={styles.infoLabel} numberOfLines={1} adjustsFontSizeToFit>Check-in</Text>
          <Text style={styles.infoValue} numberOfLines={1} adjustsFontSizeToFit>{formatDate(raw?.check_in_date)}</Text>
        </View>
      </View>
      ...
      <View style={styles.sectionCard}>
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionTitle} numberOfLines={1} adjustsFontSizeToFit>Recent Payments</Text>
        </View>
        {!raw?.rent_payments?.length ? (
          <View style={styles.emptyState}>
            <Ionicons name="receipt-outline" size={36} color="#cbd5e1" />
            <Text style={styles.emptyText} numberOfLines={1} adjustsFontSizeToFit>No payments yet</Text>
          </View>
        ) : (
          raw.rent_payments.slice(0, 3).map((p, index: number) => (
            <View key={p.s_no} style={[styles.payRow, index === raw.rent_payments.slice(0, 3).length - 1 ? { borderBottomWidth: 0 } : null]}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.payTitle} numberOfLines={1} adjustsFontSizeToFit>{formatDate(p.tenant_rent_cycles?.cycle_start)} – {formatDate(p.tenant_rent_cycles?.cycle_end)}</Text>
                <Text style={styles.payMeta} numberOfLines={1} adjustsFontSizeToFit>{p.payment_method} · {formatDate(p.payment_date)}</Text>
              </View>
              <View style={{ alignItems: 'flex-end', gap: 3 }}>
                <Text style={styles.payAmount} numberOfLines={1} adjustsFontSizeToFit>{formatAmount(p.amount_paid)}</Text>
                <View style={[styles.statusBadge, p.status === 'PAID' ? styles.statusPaid : p.status === 'PARTIAL' ? styles.statusPartial : styles.statusPending]}>
                  <Text style={[styles.statusText, p.status === 'PAID' ? styles.textStatusPaid : p.status === 'PARTIAL' ? styles.textStatusPartial : styles.textStatusPending]} numberOfLines={1} adjustsFontSizeToFit>{p.status}</Text>
                </View>
              </View>
            </View>
          ))
      ...
      >
        ...
        {newVacateDate && (
          ...
          >
            <Text style={{ fontSize: 12, fontWeight: '700', color: '#DC2626' }} numberOfLines={1} adjustsFontSizeToFit>Clear Date</Text>
          </AnimatedPressableCard>
        )}
      </SlideBottomModal>
    </>
