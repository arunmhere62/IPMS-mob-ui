...
export const PGDetailsScreen: React.FC<PGDetailsScreenProps> = ({
  navigation,
}) => {
  ...
  useEffect(() => {
    if (
      (pgDetailsResponse as any)?.success &&
      (pgDetailsResponse as any)?.data
    ) {
  ...
  const handleEditPG = () => {
    if (!canEdit) {
      Alert.alert(
        "Access Denied",
        "You don't have permission to edit PG locations"
      );
      return;
    }
  ...
  const handleDeletePG = () => {
    if (!canDelete) {
      Alert.alert(
        "Access Denied",
        "You don't have permission to delete PG locations"
      );
      return;
    }
      ...
      <View style={{ flex: 1, backgroundColor: Theme.colors.light }}>
        {isFetching ? (
          ...
          >
            ...
            {/* PG Location Header */}
            <Card
              style={{ padding: 16, marginBottom: 16, backgroundColor: "#fff" }}
            >
              <View
                style={{
                  flexDirection: "row",
                ...
                <View style={{ flex: 1 }}>
                  ...
                  >
                    <View
                      style={{
                        backgroundColor:
                          pgDetails.status === "ACTIVE" ? "#DCFCE7" : "#FEE2E2",
                        paddingHorizontal: 8,
                        paddingVertical: 4,
                        borderRadius: 4,
                      }}
            ...
            >
              ...
              <View style={{ gap: 10 }}>
                ...
                >
                  ...
                  >
                    {pgDetails.pg_type === "COLIVING"
                      ? "👥 Co-living"
                      : pgDetails.pg_type === "MENS"
                      ? "👨 Mens"
                      : "👩 Womens"}
                  </Text>
                </View>
            ...
            >
              ...
              <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
                <StatCard
                  label="Rooms"
                  value={pgDetails.room_statistics.total_rooms}
                  color={Theme.colors.text.secondary}
                  size="small"
                />
                <StatCard
                  label="Beds"
                  value={pgDetails.room_statistics.total_beds}
                  color={Theme.colors.text.secondary}
                  size="small"
                />
                <StatCard
                  label="Tenants"
                  value={pgDetails.tenant_statistics.total_tenants}
                  color={Theme.colors.text.secondary}
                  size="small"
                />
              </View>
              ...
              <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
                <StatCard
                  label="Occupancy"
                  value={`${pgDetails.room_statistics.occupancy_rate.toFixed(
                    0
                  )}%`}
                  color={
                    pgDetails.room_statistics.occupancy_rate > 75
                      ? "#10B981"
                      : pgDetails.room_statistics.occupancy_rate > 50
                      ? "#F59E0B"
                      : "#EF4444"
                  }
                  size="small"
                ...
                <StatCard
                  label="Active"
                  value={pgDetails.tenant_statistics.active_tenants}
                  color="#10B981"
                  size="small"
                />
                <StatCard
                  label="Available"
                  value={pgDetails.room_statistics.available_beds}
                  color="#3B82F6"
                  size="small"
                />
              </View>
              ...
              >
                ...
                >
                  ₹
                  {pgDetails.room_statistics.total_monthly_revenue.toLocaleString(
                    "en-IN"
                  )}
                </Text>
              </View>
